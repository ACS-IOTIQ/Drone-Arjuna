...
function normalizeGeofence(geofence: any): GeoPoint[] | null {
  ...
  const ring =
    geofence?.coordinates?.[0] ?? geofence?.geometry?.coordinates?.[0];
...
export default function LiveMap({
  ...
  const regulatoryCheckRef = useRef<{
    lat: number;
    lon: number;
  ...
  const currentRegulatoryZone = useMemo(() => {
    ...
    if (
      cached &&
      haversineMeters(cached.lat, cached.lon, frame.lat, frame.lon) <
        REGULATORY_CHECK_MIN_MOVE_M
    ) {
      return cached.rule;
    }
  ...
  const liveRouteCollisions = useMemo(() => {
    const collisions: Array<
      ReturnType<typeof findRouteCollisions>[number] & {
        droneAId: number;
        droneBId: number;
        droneAName: string;
        droneBName: string;
    ...
    for (let i = 0; i < simDroneRoutes.length - 1; i++) {
      for (let j = i + 1; j < simDroneRoutes.length; j++) {
        ...
        collisions.push(
          ...findRouteCollisions(a.route, b.route).map((collision) => ({
            ...collision,
            droneAId: a.droneId,
            droneBId: b.droneId,
  ...
  const activeCompliance = useMemo(() => {
    ...
    const geofenceExceeded =
      zoneRule.zone === "red" || Boolean(frame.geofence_breach);
    return {
      maxAltitudeM,
      maxSpeedMs,
      altitudeExceeded,
      ...
      try {
        ...
        if (frame.alt_agl > currentRegulatoryZone.maxAltitudeM) {
          const targetAlt = Math.max(
            0,
            currentRegulatoryZone.recommendedAltitudeM ||
              currentRegulatoryZone.maxAltitudeM,
          );
          await droneControlApi.command({
            drone_id: droneId,
            command: "goto",
  ...
  useEffect(() => {
    ...
    const pairs = Array.from(
      new Set(
        liveRouteCollisions.map((c) => `${c.droneAName} and ${c.droneBName}`),
      ),
    ).join(", ");
    const coords = liveRouteCollisions
      .slice(0, 3)
      .map((c) => formatCollisionCoord(c))
      .join("; ");
    const suffix =
      liveRouteCollisions.length > 3
        ? `; +${liveRouteCollisions.length - 3} more`
        : "";
    notify.warning(
  ...
  return (
    <div
      className={
        activeDetail ? "da-live-map-shell detail-open" : "da-live-map-shell"
      }
    >
      <div className="da-live-map-stage">
          ...
          {allDrones.map(({ id, frame: f }) =>
            id === droneId && frame ? (
              <Marker
                ...
                eventHandlers={
                  onSelectDrone ? { click: () => onSelectDrone(id) } : undefined
                }
              >
                <Tooltip direction="top" offset={[0, -16]}>
                  <div style={{ fontSize: 11, minWidth: 140 }}>
                    <div
                      style={{
                        fontWeight: 700,
                        color: activeCompliance?.hasViolation
                          ? "#ef4444"
                          : "#0f172a",
                      }}
                    >
                      {activeCompliance?.hasViolation
                        ? "Restriction active"
                    ...
                    {activeCompliance && (
                      ...
                      >
                        Alt {f.alt_agl.toFixed(1)} /{" "}
                        {activeCompliance.maxAltitudeM} m - Speed{" "}
                        {f.groundspeed_ms.toFixed(1)} /{" "}
                        {activeCompliance.maxSpeedMs} m/s
                      </div>
                    )}
                  </div>
                </Tooltip>
              ...
              <Tooltip permanent direction="top" offset={[0, -20]}>
                <div style={{ fontFamily: "monospace", fontSize: 11 }}>
                  ...
                  {v.speed_kts != null && (
                    <span>
                      {" "}
                      - {v.speed_kts.toFixed(1)} kts {v.heading_deg?.toFixed(0)}{" "}
                      deg
                    </span>
                  )}
                  <div
                    style={{
                      color: v.deck_status === "clear" ? "#22c55e" : "#f59e0b",
                    }}
                  >
                    deck: {v.deck_status}
                  </div>
        ...
        {((hasPosition && zoneRule) || liveRouteCollisions.length > 0) && (
          <div
            className="da-live-map-alerts"
            role="toolbar"
            aria-label="Flight advisories"
          >
              ...
              <button
                ...
                onClick={() =>
                  setActiveDetail((current) =>
                    current === "airspace" ? null : "airspace",
                  )
                }
            ...
            {liveRouteCollisions.length > 0 && (
              <button
                ...
                onClick={() =>
                  setActiveDetail((current) =>
                    current === "collisions" ? null : "collisions",
                  )
                }
              ...
              >
                ...
                <span>
                  {liveRouteCollisions.length} path conflict
                  {liveRouteCollisions.length === 1 ? "" : "s"}
                </span>
              </button>
            )}
          </div>
      ...
      {activeDetail && (
        <aside
          className="da-live-map-detail"
          aria-label={
            activeDetail === "airspace"
              ? "Airspace status"
              : "Path collision points"
          }
        >
          <header>
            <strong>
            ...
            {activeDetail === "airspace" && frame && zoneRule && (
              <div className="da-live-map-detail-section">
                ...
                <div
                  className={
                    activeCompliance?.hasViolation
                      ? "da-live-zone-summary is-danger"
                      : "da-live-zone-summary"
                  }
                >
                  <ShieldAlert size={17} />
                  <div>
                ...
                <div className="da-live-limit-grid">
                  <div>
                    <span>Altitude</span>
                    <strong>{frame.alt_agl.toFixed(1)} m</strong>
                    <small>
                      Limit{" "}
                      {activeCompliance?.maxAltitudeM ?? zoneRule.maxAltitudeM}{" "}
                      m
                    </small>
                  </div>
                  <div>
                    <span>Speed</span>
                    <strong>{frame.groundspeed_ms.toFixed(1)} m/s</strong>
                    <small>
                      Limit{" "}
                      {activeCompliance?.maxSpeedMs ?? zoneRule.maxSpeedMs} m/s
                    </small>
                  </div>
                </div>
                ...
                {activeCompliance?.altitudeExceeded && (
                  <div className="da-live-violation">
                    Altitude limit exceeded
                  </div>
                )}
                {activeCompliance?.speedExceeded && (
                  <div className="da-live-violation">Speed limit exceeded</div>
                )}
                {activeCompliance?.geofenceExceeded && (
                  <>
                    <div className="da-live-violation">
                      Geofence block active / automatic RTL protection enabled
                    </div>
                    {onManualControlRequest && (
                      <button
                        type="button"
            ...
            {activeDetail === "collisions" && (
              <div className="da-live-map-detail-section">
                <div className="da-live-collision-summary">
                  ...
                  <span>
                    {liveRouteCollisions.length} collision point
                    {liveRouteCollisions.length === 1 ? "" : "s"} detected
                  </span>
                </div>
                {liveRouteCollisions.map((collision, index) => (
                  <div
                    className="da-live-collision-row"
                    key={
                      "live-collision-" +
                      collision.droneAId +
                      "-" +
